...
from util.ddpg_env import RecommendENV
            ...
            with tf.Session(config=config) as o_sess:
                ...
                for ii in range(MEMORY_CAPACITY):
                    ...
                    s_, emb_s_, r = env.step(user_id=user_id,
                                             in_state=s,
                                             in_a_w=a_w,
                                             select_size=self.cand_size,
                                             train_percent=positive_percent)
...
if __name__ == '__main__':
    ...
    cand_size = 50  # Number of randomly selected candidate set items, make sure select_size < len(c_items_list)
